...
def _box_loss(box_outputs, box_targets, num_positives, delta: float = 0.1):
    ...
    box_loss = smooth_l1_loss(box_outputs, box_targets, weights=mask, beta=delta, size_average=False)
...
def loss_fn(
    ...
    box_loss = box_loss_weight * torch.sum(torch.stack(box_losses, dim=-1), dim=-1)
...
class DetectionLoss(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.delta = config.smoothl1_beta   # config.delta
